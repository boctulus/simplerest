...
# Si no se proporciona un valor para $OutputFile, usar "packages-" como prefijo
if (-not $OutputFile) {
    $OutputFile = "packages.zip"
}

# Función para obtener las exclusiones desde .zipignore
function Get-ZipIgnorePatterns {
    param([string]$ZipIgnoreFile)
    # Buscar .zipignore en el directorio actual
    $zipIgnorePath = Join-Path $PWD $ZipIgnoreFile
    if (Test-Path $zipIgnorePath) {
        Get-Content $zipIgnorePath | ForEach-Object {
...
# Generar el archivo ZIP del directorio packages recursivamente
Write-Host "Creando el archivo ZIP del directorio packages con las exclusiones..."
& 7z a -tzip $OutputFile packages\* -r -x@"$exclusionsFile"
