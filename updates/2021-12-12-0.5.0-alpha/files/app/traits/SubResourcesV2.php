...
trait SubResourcesV2
{
    static function getSubResources(string $table, Array $connect_to, ?Object &$instance = null, ?string $tenant_id = null)
    {
        ...
        */
        ...
        //         if (!preg_match('/([a-z0-9_]+)'.$sep.'([a-z0-9_]+)/i', $field, $matches)){
        //             throw new \Exception("Unexpected error trying to parse '$field'");
        //         }

        //         $tt = $matches[1];
        //         $ff = $matches[2];

        //         // Nesting ! <---  tiene que depender del config
        //         if ($tt == $table){
        //             $res[$ix][$ff] = $val;
        //         } else {
        //             $res[$ix][$tt][$ff] = $val;
