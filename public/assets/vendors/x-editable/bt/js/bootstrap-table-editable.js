	        ...
	        column.formatter = function (value, row, index) {
	          ...
	          if (noEditFormatter === false) {
	          	const $a = $(document.createElement("a"))
	          			...
	          			.attr("data-name", column.field)
	          			.attr("data-pk", row[_this.options.idField])
	          			.text(result);

	          	if (editableOpts.value == null) {
	          		$a.attr("data-value", (column.editable.include_formatter === false) ? value : result) // Allow the formatter to not affect the input value
	          	}

	          	$__default['default'].each(editableOptions, function (key, value) {
					$a.attr(editableOptions);
				});
