...
if (in_array($_GET['a'],$allowed)){
	...
	\Libs\Factory::response()->sendError('Incorrect action');
	exit();
}
...
function signup()
{
	...
	try {
		...
		if (count($u->filter(['id'],['email'=>$data['email']]))>0)
			\Libs\Factory::response()->sendError('Email already exists');
		...
		if (empty($u->create($data)))
			\Libs\Factory::response()->sendError("Error in user registration!");
		...
		\Libs\Factory::response()->send(['token'=>$token, 'exp' => $payload['exp'] ]);
...
function login()
{
	switch($_SERVER['REQUEST_METHOD']) {
		...
		case 'POST':
			$data  = \Libs\Factory::request()->getBody(false);
		...
		default:
			\Libs\Factory::response()->sendError('Incorrect verb',405);
		break;	
	}	
	
	if (empty($email)){
		\Libs\Factory::response()->sendError('email is required',400);
	}else if (empty($password)){
		\Libs\Factory::response()->sendError('password is required',400);
	}
	...
	if ($u->checkUserAndPass()){
		...
		\Libs\Factory::response()->sendError("User or password are incorrect", 401);
}
...
function renew()
{
	if ($_SERVER['REQUEST_METHOD']=='OPTIONS'){
		// passs
		\Libs\Factory::response()->send('OK',200);
	...
	try {
		...
		if($jwt)
		{
			...
			\Libs\Factory::response()->sendError('Token not found',400);
		}
	} catch (\Exception $e) {
		\Libs\Factory::response()->sendError($e->getMessage(), 400);
	}	
}