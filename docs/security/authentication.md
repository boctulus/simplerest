...
The controller calls the response error path with HTTP 401 for incorrect login credentials, invalid API keys, and invalid or expired JWTs. Disabled or pending accounts use HTTP 403 in the relevant login/check paths. The automatic API callable gate also uses 403 when an action is not in the controller's callable list. These are source-level status arguments; this phase did not probe the emitted HTTP response body or a running server.

If an Authorization value is present but does not contain a parsed Bearer token, the core JWT path calls the error response with 400. Other malformed forms can follow the JWT decode/error path; the exact wire response has not been probed.
...
- Existing test sources include [HTTP auth tests](../../unit-tests/auth/AuthTest.php#L180) and application-specific [password-reset tests](../../unit-tests/auth/PasswordResetTest.php#L70). They were inspected but not run in this audit.
