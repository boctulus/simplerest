...
Focused tests verify that, for a compiled context with a literal <code>*</code> allow, generic <code>can()</code> grants the permission on another resource while <code>hasResourcePermission()</code> denies it. An uncompiled legacy context denies through both paths. Compiled wildcard denies block both paths. The automatic API controller uses <code>hasResourcePermission()</code> and named special-permission checks, so its literal-star behavior differs from generic <code>can()</code>. The framework's intended contract for literal-star rules remains unresolved; see the open [wildcard behavior issue](../issues/acl-literal-wildcard-path-divergence.md). Explicit <code>read_all</code>/<code>write_all</code> grants are a distinct compiled capability mechanism.
