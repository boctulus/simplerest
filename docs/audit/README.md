...
| `framework/QueryBuilder.md` | Fluent CRUD, joins, related resources, pagination, raw queries, transactions, soft deletes, execution modes, caching, and model hooks | Pending the separate Query Builder audit; claims are not endorsed by the legacy page location |
| `framework/AutoJoins.md` | Schema-inferred joins, N:M pivot joins, qualification, and supported join types | Pending automatic-join audit after Query Builder and schemas |
| `framework/PGSQL-Compatibility.md` | PostgreSQL DQL/DDL matrix, method compatibility, known issues, and test-running instructions | Pending per-operation driver audit; test instructions are not verified |
| `framework/Query-Builder-Comparison.md` | Laravel feature matrix, memory/performance ratios, and comparative recommendations | Feature inventory pending; numeric performance claims rejected absent reproducible benchmark evidence |
...
Installation/bootstrap, routing/request lifecycle, API resolution, and database connection selection now have claim-level audits. Neither installation workflow is verified end to end; route-definition examples, request/response helper APIs, live database connectivity, and Query Builder behavior remain pending. Continue with the Query Builder audit, then schemas and automatic API resolution; authentication and ACL; CLI and migrations; integrations; deployment; and performance. For each phase, capture implementation, configuration, tests, and runnable evidence before publishing a verified status.
