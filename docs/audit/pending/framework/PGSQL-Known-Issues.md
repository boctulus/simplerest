...
> **Audit status:** Historical issue report, not current PostgreSQL compatibility guidance. Its “DQL functional,” “DDL not supported,” and dated fix statuses have not been revalidated against current source or executed tests in this audit. See the [claim-level findings](../../README.md#claim-level-findings-query-builder) and [legacy compatibility matrix](./PGSQL-Compatibility.md).

> **State:** DQL functional, DDL not supported.
> **Last updated:** 2026-05-26 (issues 2-8 fixed)
> See the [pending legacy compatibility matrix](./PGSQL-Compatibility.md); its claims and test instructions are not current guidance.
