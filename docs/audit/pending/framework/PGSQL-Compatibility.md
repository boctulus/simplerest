...
> **Audit status:** This entire compatibility matrix remains a legacy audit input. The connection layer has a `pgsql` PDO construction branch and the repository contains PostgreSQL test source, but this audit ran no PostgreSQL tests and did not verify the matrix or its old “running tests” instructions. Per-operation Query Builder and schema/DDL compatibility remains unresolved. See the [claim-level audit](../../README.md#claim-level-findings-query-builder).
