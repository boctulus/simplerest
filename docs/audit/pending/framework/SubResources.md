...
> **Audit status:** This legacy page is quarantined. The Query Builder's `connectTo()`/`getSubResources()` read dispatch and two test-source assertions have been inspected, but no test ran. Its `insertWithSubResources()` example is rejected: `InsertWithSubResourcesTrait` has no such method, and `insertStruct()` stops at a debug call/TODO without writing records. Nested API route behavior, authorization, and relationship/schema resolution remain unverified. See the [claim-level audit](../../README.md#claim-level-findings-query-builder); continue relation work with the schema and API audit.
...
- [`AutoJoins.md`](./AutoJoins.md) — legacy automatic-join claims (pending audit)
- [`AutomaticEndpoints-Summary.md`](./AutomaticEndpoints-Summary.md) — pending audit input
