...
> **Audit status:** This is quarantined legacy input, not the canonical Query Builder guide. The claim-level audit confirms selected query-construction paths only; it does not verify the page's full CRUD, transaction, raw-SQL, hook, execution-mode, related-resource, or driver claims. The narrow current account is in [Database Query Builder](../../../database/query-builder.md); unresolved and rejected claims are listed in the [audit register](../../README.md#claim-level-findings-query-builder). Performance and blanket Laravel-compatibility assertions are not current guarantees.
