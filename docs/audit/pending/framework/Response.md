...
> **Legacy disposition:** retained under `docs/audit/pending/`, not a current `Response` API guide. This phase traced only `Response::flush()` in the lifecycle; output formats, helpers, and immutable-method claims remain pending. See the [claim-level audit](../../README.md#claim-level-findings-routing-and-request-lifecycle).
