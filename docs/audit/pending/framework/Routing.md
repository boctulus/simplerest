...
> **Legacy disposition:** retained under `docs/audit/pending/`, not current routing documentation. The resolver/dispatch claims are audited in [request lifecycle](../../../core/request-lifecycle.md); route examples, package routing, CLI behavior, and remaining claims stay pending. See the [claim-level audit](../../README.md#claim-level-findings-routing-and-request-lifecycle).
