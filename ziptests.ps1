...
# Si no se proporciona un valor para $OutputFile, usar "tests-" como prefijo
if (-not $OutputFile) {
    $OutputFile = "tests.zip"
}
...
# Generar el archivo ZIP del directorio tests recursivamente
Write-Host "Creando el archivo ZIP del directorio tests con las exclusiones..."
& 7z a -tzip $OutputFile tests\* -r -x@"$exclusionsFile"
